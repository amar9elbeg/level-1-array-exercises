...
function getFlavorByIndex(originalFlavors, i) {
  for (let index = 0; index < originalFlavors.length; index++) {
...
For example, removeFlavorByName(originalFlavors, "Vanilla") 
would return an array with the length 30 including all of the flavors except Vanilla. 

Hint: You can use .filter() for this

*/
function removeFlavorByName(originalFlavors, flavor) {
  const flavors = originalFlavors.filter((element) => {
...
let newArray;
function copy(newArray, originalFlavors) {
  let clone = newArray.concat(originalFlavors);
  return clone;
}
console.log(newArray, originalFlavors);
...
all of their chocolate flavors. Write a function that checks every item in the array for a given string and returns
 ...
 pass a string as a parameter, and invoke with the argument "chocolate". This way you could also filter for "Vanilla",
...
function filterByWord(originalFlavors, flvr) {
  // const chocolateFlavor = originalFlavors.map((flavor) => {
  //   if (flavor.includes(flvr)) {
  //     return flavor;
  //   }
  // });
  // return chocolateFlavor.filter((flavor) => flavor !== undefined);
  return originalFlavors.filter((flavor) => {
    if (flavor.includes(flvr)) {
      return flavor;
    }
  });
...
function getAverageWordLength(originalFlavors) {
  sum = 0;
  originalFlavors.forEach((element) => {
...
function getRandomFlavors(
  ...
  const allFlavors = originalFlavors.concat(
    newFlavors,
    seasonalFlavors,
    regionalFlavors
  );
  for (let index = 0; index < 31; index++) {
    ...
    randomFlavorArray.push(allFlavors[randomNumber]);
  }
